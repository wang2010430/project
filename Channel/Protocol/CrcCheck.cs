...
namespace Channel
{
    ...
    public class CRCValue
    {
        ...
        UInt32[] values = new UInt32[MaxByteValues];
        ...
        public CRCValue(int BitCount, UInt32 Polynominal, bool ShiftRight,
            UInt32 Initial, UInt32 FinalMask)
        {
            // BitCount    : CRC Size
        ...
        UInt32 ForwardTableEntry(UInt32 entryindex)
        {
            ...
            UInt32 result = entryindex << (BITCOUNT - BitSperByte);
        ...
        UInt32 ReverseTableEntry(UInt32 entryindex)
        {
        ...
        void update(byte[] buffer, int length)
        {
            ...
            else
            {
                for (int ii = 0; ii < length; ++ii)
                {
                    UInt32 index = ((crc_register >> (BITCOUNT - BitSperByte)) ^ buffer[ii]);
        ...
        public UInt32 GetCrc32(byte[] buffer, int length)
        {
            reset();
        ...
        public UInt32 GetCrc(byte[] buffer, int length)
        {
            return GetCrc32(buffer, length);
        }

        public UInt16 GetCrc16(byte[] buffer, int length)
        {
            return (UInt16)GetCrc32(buffer, length);
        }
    ...
    public class CrcCheck : CrcCheckBase
    {
        ...
        protected virtual void CountCheckCode(byte[] checkData, out byte high, out byte low)
        {
            ...
            // 计算校验码
            uint ret32bit = crcValue.GetCrc(dd, dd.Length);
        ...
        {
            // 计算校验码
            ret32bit = crcValue.GetCrc(checkData, checkData.Length);
        }
        #endregion
    }
